...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{img/arquitectura-mvc.png}
...
La capa de modelo encapsula toda la lógica de negocio y se subdivide en dos componentes principales:

\begin{enumerate}
    \item \textbf{Clases del Dominio:} Representan las entidades del negocio con sus atributos y comportamientos. Incluyen:
    \begin{itemize}
        \item \texttt{Usuario}, \texttt{Administrador}, \texttt{Cajero}, \texttt{Cliente} - Jerarquía de usuarios
        \item \texttt{Producto} - Artículos del cafetín con control de stock
        \item \texttt{Pedido}, \texttt{DetallePedido} - Gestión de órdenes
        \item \texttt{Inventario} - Control de niveles de stock
    \end{itemize}
    
    \item \textbf{Capa DAO (Data Access Object):} Abstrae el acceso a la base de datos:
    \begin{itemize}
        \item \texttt{ConexionBD} - Gestión de conexiones (patrón Singleton)
        \item \texttt{UsuarioDAO}, \texttt{ProductoDAO}, \texttt{PedidoDAO} - Operaciones CRUD específicas
    \end{itemize}
\end{enumerate}

\subsubsection{Capa de Controlador}

Los Servlets actúan como controladores que procesan las peticiones HTTP:

\begin{itemize}
    \item \texttt{ServletLogin} - Autenticación y gestión de sesiones
    \item \texttt{ServletPedidos} - Procesamiento de pedidos
    \item \texttt{ServletProductos} - Gestión del catálogo
    \item \texttt{ServletUsuario} - Administración de usuarios
\end{itemize}

Cada Servlet sigue el patrón: \texttt{doPost()} recibe parámetros, determina la acción, invoca métodos del modelo, y redirige a la vista apropiada.

\subsubsection{Capa de Vista}

Las páginas JSP presentan la interfaz de usuario organizada por roles:

\begin{itemize}
    \item \textbf{Módulo de Autenticación:} \texttt{login.jsp}, \texttt{registro.jsp}
    \item \textbf{Módulo del Cliente:} \texttt{menu.jsp}, \texttt{carrito.jsp}, \texttt{mis-pedidos.jsp}
    \item \textbf{Módulo Administrativo:} \texttt{dashboard.jsp}, \texttt{productos.jsp}, \texttt{pedidos.jsp}
\end{itemize}

\subsubsection{Flujo de Comunicación}

El flujo típico de una operación sigue estos pasos:

\begin{enumerate}
    \item Usuario interactúa con una \textbf{Vista (JSP)}
    \item Vista genera petición HTTP al \textbf{Controlador (Servlet)}
    \item Controlador invoca métodos del \textbf{Modelo}
    \item Modelo accede a \textbf{Base de Datos} vía DAO
    \item DAO retorna resultados al Controlador
    \item Controlador selecciona Vista y envía respuesta
\end{enumerate}

...
La Programación Orientada a Objetos (POO) es un paradigma que estructura el software en torno a objetos que combinan datos y comportamiento. Se basa en cuatro principios fundamentales para crear sistemas modulares y reutilizables \cite{booch2007}.

El \textbf{encapsulamiento} oculta los detalles internos de un objeto, exponiendo solo una interfaz pública. La \textbf{herencia} permite crear nuevas clases a partir de otras existentes, fomentando la reutilización de código. El \textbf{polimorfismo} posibilita que objetos de diferentes clases respondan de manera específica al mismo mensaje. La \textbf{abstracción} identifica las características esenciales de un objeto, ignorando detalles no relevantes \cite{bloch2018}.

\subsection{Diagramas UML}

El Lenguaje Unificado de Modelado (UML) es un estándar para visualizar y documentar sistemas software \cite{fowler2003}. Los diagramas de clases muestran la estructura estática del sistema mediante clases, sus atributos, métodos y las relaciones entre ellas. Son cruciales en las fases de análisis y diseño.

\subsection{Relaciones entre Clases en POO}

Las relaciones definen cómo interactúan los objetos. La \textbf{asociación} indica una conexión básica entre clases. La \textbf{agregación} representa una relación "todo-parte" con independencia. La \textbf{composición} es una agregación fuerte donde la parte depende del todo. La \textbf{herencia} establece una relación "es-un" entre una superclase y sus subclases \cite{larman2004}.

\subsection{MySQL y Bases de Datos Relacionales}

MySQL es un sistema de gestión de bases de datos relacionales (RDBMS) que organiza la información en tablas relacionadas. Utiliza SQL (Structured Query Language) para definir, manipular y consultar datos, garantizando integridad y consistencia \cite{dubois2013}. \textbf{XAMPP} es una distribución que incluye MySQL, facilitando la configuración de un entorno de desarrollo local.

\subsection{Entorno de Desarrollo Java Web}

Se utilizó \textbf{Apache NetBeans} como IDE, con \textbf{Ant} para la automatización del proceso de construcción. El despliegue y ejecución se realizó en el servidor \textbf{Apache Tomcat}, que es un contenedor de servlets y JSP para aplicaciones web Java \cite{hall2011}.

\subsection{Patrón de Arquitectura MVC}

El patrón Modelo-Vista-Controlador (MVC) separa la aplicación en tres componentes: el \textbf{Modelo} (lógica y datos), la \textbf{Vista} (presentación) y el \textbf{Controlador} (intermediario que gestiona las peticiones). Esta separación facilita el mantenimiento y la escalabilidad \cite{gamma1994}.

\subsection{Patrón DAO (Data Access Object)}

El patrón DAO abstrae y encapsula todo el acceso a la fuente de datos (como una base de datos), proporcionando una interfaz simple para realizar operaciones CRUD (Crear, Leer, Actualizar, Eliminar). Esto separa la lógica de negocio de la lógica de persistencia \cite{alur2003}.

\subsection{Servlets y JSP}

Los \textbf{Servlets} son componentes Java en el servidor que extienden sus capacidades para procesar peticiones HTTP y generar respuestas dinámicas. \textbf{JavaServer Pages (JSP)} es una tecnología que permite crear páginas web dinámicas mezclando HTML con código Java. En conjunto, son la base para implementar el controlador y la vista en aplicaciones web Java con MVC \cite{hall2011}.
